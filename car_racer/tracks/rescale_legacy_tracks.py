"""
Одноразовый скрипт: масштабировал 5 "легаси" трасс без реальной калибровки
...
С тех пор points_reserve.txt/points_reserve2.txt удалены (геометрия признана
неудачной), points_oval.txt пересобран заново под реальные метры и
переименован в points_oval_half_mile.txt (уже НЕ через этот скрипт/
SCALE_FACTOR), points_stadium_chicane.txt удалён - из исходного списка
дожил только points.txt (см. CLAUDE.md, раздел "Масштаб треков"). TRACKS
ниже сокращён соответственно - остальные оставлены только в этом докстринге
как историческая справка (см. таблицу длин ниже), не как рабочий список.
...
Результат уже применён и закоммичен - повторный запуск смасштабирует
points.txt ЕЩЁ раз поверх уже применённого масштаба (не идемпотентно,
испортит файл - SCALE_FACTOR применится к уже отмасштабированным
координатам), поэтому обычно не нужен. Оставлен в репозитории как
документация того, как получены текущие точки (по аналогии с
...
# Изначально здесь было 5 файлов (см. докстринг выше) - остальные 4 с тех
# пор удалены/пересобраны и переименованы, список сокращён до того, что
# реально ещё существует под этими именами.
TRACKS = ["points.txt"]
SCALE_FACTOR = 2.0
